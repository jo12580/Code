...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	switch(GPIO_Pin)
	{
		...
		case GPIO_PIN_6:
		{
			if(key_in==1)		key_exti=11;
			if(key_in==2)		key_exti=12;
			if(key_in==3)		key_exti=13;
		}break;
		case GPIO_PIN_7:
		{
			if(key_in==1)		key_exti=21;
			if(key_in==2)		key_exti=22;
			if(key_in==3)		key_exti=23;
		}break;
	}
}
